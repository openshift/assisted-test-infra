...
usage() {
  cat <<USAGE
Usage: $0 --old-secondary-ip <ip> --new-secondary-ip <ip> --new-machine-network <cidr> --new-gateway-ip <ip> --primary-ip <ip> [--ssh-user core] [--ssh-port 22] [--ssh-key ~/.ssh/id_rsa] [--ssh-strict-hostkey-checking no] [--new-dns-server <ip>]
...
Required:
  --old-secondary-ip    Existing (pre-change) secondary IP address to connect to initially
  --new-secondary-ip    New secondary IP address to wait for and connect to after first reboot
  --new-machine-network  CIDR for the network of the new IP (e.g. 192.168.201.0/24 or fd00::/64)
  --new-gateway-ip      Gateway IP for the new network
  --primary-ip          Primary IP address to SSH to initially

Optional:
  --new-dns-server      DNS server for the new network
...
main() {
  ...
  local remote_cmd
  remote_cmd=("sudo" "bash" "${remote_dir}/secondary-node-actions.sh"
    "--new-ip" "$NEW_SECONDARY_IP"
    "--new-machine-network" "$NEW_MACHINE_NETWORK"
    "--new-gateway-ip" "$NEW_GATEWAY_IP"
  )
  
  # Add optional DNS server parameter if provided
  if [[ -n "$NEW_DNS_SERVER" ]]; then
    remote_cmd+=("--new-dns-server" "$NEW_DNS_SERVER")
  fi
  
  ssh_exec "$SSH_USER" "$PRIMARY_IP" "$SSH_PORT" "$SSH_KEY" "$SSH_STRICT_HOSTKEY_CHECKING" "${remote_cmd[@]}" || true
