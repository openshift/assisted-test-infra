...
usage() {
  cat <<USAGE
Usage: $0 --old-ip <ip> --new-ip <ip> --new-machine-network <cidr> [--ssh-user core] [--ssh-port 22] [--ssh-key ~/.ssh/id_rsa] [--ssh-strict-hostkey-checking no]
...
main() {
  ...
  ssh_exec "$SSH_USER" "$OLD_IP" "$SSH_PORT" "$SSH_KEY" "$SSH_STRICT_HOSTKEY_CHECKING" sudo bash "${remote_dir}/secondary-node-actions.sh" --new-ip "$NEW_IP" --new-machine-network "$NEW_MACHINE_NETWORK" || true
