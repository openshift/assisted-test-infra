...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help) echo "Usage: $0 --old-ip <ip> --new-ip <ip> --new-machine-network <cidr> --new-gateway-ip <ip> --kubeconfig-path <path> --recert-image-tar <path> [--new-dns-server <ip>] [--ssh-user ...] [--ssh-port ...] [--ssh-key ...]"; exit 0;;
    *) shift 1;;
  esac
done
...
log "Invoking single-ip-change.sh with provided arguments"
"${SCRIPT_DIR}/flows/single-ip-change.sh" "${PASS_ARGS[@]}"

# Reboot old node IP and wait for SSH on the new IP
OLD_SSH_IP="$OLD_IP"
NEW_SSH_IP="$NEW_IP"
...
log "Single-stack IP change test completed successfully. New IP: ${NEW_IP}"
