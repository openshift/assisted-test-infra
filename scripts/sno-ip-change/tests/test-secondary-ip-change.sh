...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 --old-ip <ip> --new-ip <ip> --new-machine-network <cidr> --new-gateway-ip <gateway> [--new-dns-server <dns>] --kubeconfig-path <path> [--ssh-user ...] [--ssh-port ...] [--ssh-key ...] [--ssh-strict-hostkey-checking yes|no]";
      exit 0;;
    *) shift 1;;
  esac
done
