...
if [ "${DEPLOY_TARGET}" == "minikube" ]; then
    ...
    wait_for_url_and_run "$(minikube service ${PROMETHEUS_SERVICE_NAME} -p $PROFILE -n ${NAMESPACE} --url)" "echo \"waiting for ${PROMETHEUS_SERVICE_NAME}\""
    ...
    wait_for_url_and_run "http://${NODE_IP}:${PROMETHEUS_UI_PORT}" "spawn_port_forwarding_command $PROMETHEUS_SERVICE_NAME $PROMETHEUS_UI_PORT $NAMESPACE $NAMESPACE_INDEX $PROFILE $KUBECONFIG minikube"
    ...
    wait_for_url_and_run "${CLUSTER_UI_URL}" "spawn_port_forwarding_command $PROMETHEUS_SERVICE_NAME $OCP_PROMETHEUS_UI_PORT $NAMESPACE $NAMESPACE_INDEX $PROFILE $OCP_KUBECONFIG ocp ${CLUSTER_VIP} ${SERVICE_NODEPORT}"
