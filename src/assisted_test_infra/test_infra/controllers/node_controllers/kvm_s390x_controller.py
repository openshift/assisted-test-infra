...
class KVMs390xController(LibvirtController):
    ...
    def _create_nodes(self, running=True):
        ...
        if x:
            ...
            command = f"scp {self.get_download_path()} {self.libvirt_uri[y.start() + 2: x.start()]}:" + \
    ...
    def check_vms_for_first_reboot_and_start(self):
        log.debug("Restart vms after firstboot. masters_count/workers_count: %d/%d",
                  self._config.masters_count, self._config.workers_count)
