...
class OciApiController(NodeController):
    ...
    def __init__(self, config: BaseNodesConfig, cluster_config: BaseClusterConfig):
        ...
        self.oci_compartment_oicd = self._config.oci_compartment_oicd
    ...
    def _initialize_oci_clients(self):
        ...
        try:
            ...
            self.object_storage_client = oci.object_storage.ObjectStorageClient(self._config.get_provider_config())
            self.compute_client = oci.core.ComputeClient(self._config.get_provider_config())
            self.volume_client = oci.core.BlockstorageClient(self._config.get_provider_config())
            # resource manager for stack creation and job
            self.resource_manager_client = oci.resource_manager.ResourceManagerClient(
                self._config.get_provider_config())
            # wrapper for resource_manager_client with waiters
            self.resource_manager_client_composite_operations = (
                oci.resource_manager.ResourceManagerClientCompositeOperations(self.resource_manager_client))
        except Exception as e:
            raise e

    def _create_bucket(self, name: str, public_access_type: str = "NoPublicAccess", storage_tier: str = "Standard",
                       object_events_enabled: bool = False, versioning: str = "Disabled",
                       auto_tiering: str = "Disabled", **kwargs):
        bucket_details = {
            "name": name,
            "compartment_id": self.oci_compartment_oicd,
        ...
        name_space = self.object_storage_client.get_namespace().data

        log.info(f"Create oci bucket {create_bucket_details}")
        obj = self.object_storage_client.create_bucket(name_space, create_bucket_details)
        self._cleanup_resources.append(CleanupResource(self.object_storage_client.delete_bucket, name_space, name))
        assert obj.status == 200
        # Need the name_space and bucket name
        return name_space, name

    def _upload_iso_to_bucket(self, file_path: str, name_space: str, bucket_name: str):
        log.info(f"Upload iso file to bucket object storage {file_path}")
        if os.path.isfile(file_path):
            try:
                self.object_storage_client.put_object(name_space, bucket_name, os.path.basename(file_path),
                                                      open(file_path, "rb"))
                self._cleanup_resources.append(CleanupResource(self.object_storage_client.delete_object,
                                                               name_space, bucket_name, os.path.basename(file_path)))
            except Exception as e:
                raise e
        else:
            raise RuntimeError(f"Could not find {file_path}")

    def _create_pre_authenticated(self, name: str, file_path: str, name_space: str, bucket_name: str,
        ...
        obj = self.object_storage_client.create_preauthenticated_request(
            namespace_name=name_space, bucket_name=bucket_name,
            create_preauthenticated_request_details=pre_authenticated_req)
        self._cleanup_resources.append(CleanupResource(self.object_storage_client.delete_preauthenticated_request,
                                                       name_space, bucket_name, obj.data.id))

        assert obj.status == 200
        return obj.data.full_path

    def _terraform_variables(self, cluster_name: str, image_source_uri: str, control_plane_shape: str,
                             compute_shape: str, compute_count: str, base_dns: str, **kwargs) -> dict:
        ...
        BM.Standard3.64 shape refers to boot from iscsi
        """
        variables = {
            'compartment_ocid': self.oci_compartment_oicd,
    ...
    @staticmethod
    def _base64_zip_file(file_zip: str) -> base64:
    ...
    def _create_stack(self, name: str, name_space: str, bucket_name: str, terraform_zip_path: str,
                      terraform_variable: dict = {}, timeout_seconds: int = 1200) -> str:
        ...
        try:
            obj = self.resource_manager_client_composite_operations.create_stack_and_wait_for_state(
                create_stack_details, wait_for_states=[OciState.ACTIVE.value],
                waiter_kwargs={"max_wait_seconds": timeout_seconds, "succeed_on_not_found": False})
            self._cleanup_resources.append(CleanupResource(
                self.resource_manager_client_composite_operations.delete_stack_and_wait_for_state, obj.data.id))
        except Exception as e:
            log.info(f"failed to create stack {e}")
            raise
        return obj.data.id

    def _apply_job_from_stack(self, stack_id: str, display_name: str, timeout_seconds: int = 1800) -> dict:
        ...
        try:
            ...
            self._cleanup_resources.append(CleanupResource(
                self.resource_manager_client_composite_operations.create_job_and_wait_for_state, destroy_job_details,
                wait_for_states=[OciState.SUCCEEDED.value],
                waiter_kwargs={"max_wait_seconds": timeout_seconds, "succeed_on_not_found": False}))

            job = self.resource_manager_client_composite_operations.create_job_and_wait_for_state(
                create_job_details, wait_for_states=[OciState.FAILED.value, OciState.SUCCEEDED.value],
                waiter_kwargs={"max_wait_seconds": timeout_seconds, "succeed_on_not_found": False})
            log.info(f"Job run ended with {job.data.lifecycle_state} state")
            log.info(self.resource_manager_client.get_job_logs_content(job.data.id).data)
        ...
        items = self.resource_manager_client.list_job_outputs(job.data.id).data.items
        for item in items:
    ...
    @staticmethod
    def _waiter_status(client_callback: Callable, name: str, status: str, **callback_kwargs):
        def is_status():
    ...
    def prepare_nodes(self):
        ...
        name_space, bucket_name = self._create_bucket(random_name("bucket-"))
        self._upload_iso_to_bucket(self._entity_config.iso_download_path, name_space, bucket_name)
        url_path = self._create_pre_authenticated(random_name("preauth-"),
                                                  self._entity_config.iso_download_path, name_space, bucket_name)

        terraform_variables = self._terraform_variables(cluster_name=self._entity_config.entity_name,
                                                        image_source_uri=url_path,
                                                        control_plane_shape=self._config.oci_controller_shape,
        ...
        stack_id = self._create_stack(random_name("stack-"), name_space, bucket_name,
                                      self._config.oci_infrastructure_zip_file, terraform_variables)
