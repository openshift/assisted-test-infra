...
class Agent(BaseCustomResource):
    ...
    def unbind(self) -> None:
        ...
        log.info("Unbinding agent %s with the following update: %s", self.ref, pformat(body))
    ...
    def _are_agents_in_state(agents: List["Agent"], expected_state: str) -> bool:
        ...
        log.info(
            f"Waiting for agents to have the state '{expected_state}'" f"and currently agent state is {agent_states}"
        )
