locals {
  cluster_ingress_domain = "*.apps.${var.cluster_name}.${var.oci_dns_zone_name}"
  cluster_api_domain     = "api.${var.cluster_name}.${var.oci_dns_zone_name}"
  cluster_api_int_domain = "api-int.${var.cluster_name}.${var.oci_dns_zone_name}"
}

resource "oci_dns_rrset" "dns_rrset_ingress" {
  #Required
  zone_name_or_id = var.oci_dns_zone_name
