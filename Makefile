...
_install_cluster:
	discovery-infra/install_cluster.py -id $(CLUSTER_ID) -ps '$(PULL_SECRET)' -ns $(NAMESPACE) -t $(TARGET) --oc-token $(OC_TOKEN) --oc-server $(OC_SERVER) --service-name $(SERVICE_NAME)
...
destroy_nodes:
	skipper run 'discovery-infra/delete_nodes.py -iU $(REMOTE_SERVICE_URL) -id $(CLUSTER_ID) -ns $(NAMESPACE) -t $(TARGET) --oc-token $(OC_TOKEN) --oc-server $(OC_SERVER) --service-name $(SERVICE_NAME)' $(SKIPPER_PARAMS)
...
delete_all_virsh_resources: destroy_nodes delete_minikube
	skipper run 'discovery-infra/delete_nodes.py -ns $(NAMESPACE) -t $(TARGET) --oc-token $(OC_TOKEN) --oc-server $(OC_SERVER) --service-name $(SERVICE_NAME) -a' $(SKIPPER_PARAMS)
