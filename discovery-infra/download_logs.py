    ...
    if args.cluster_id:
        ...
        for cluster in clusters:
            if args.download_all or should_download_logs(cluster):
                download_logs(
                    client, cluster, args.dest, args.must_gather, args.update_by_events
                )
...
def is_update_needed(
    ...
    with tempfile.NamedTemporaryFile() as latest_event_tp:
        ...
        if filecmp.cmp(destination_event_file_path, latest_event_tp.name):
            ...
            log.info(
                "update needed, new events found, deleting {} ".format(
...
def download_logs(
    ...
    try:
        write_metadata_file(
            client, cluster, os.path.join(output_folder, "metdata.json")
        )

        with suppressAndLog(
            AssertionError, ConnectionError, requests.exceptions.ConnectionError
        ):
            client.download_metrics(os.path.join(output_folder, "metrics.txt"))

        for ignition_file in (
            "bootstrap.ign",
            "master.ign",
            "worker.ign",
            "install-config.yaml",
        ...
        for host_id in map(lambda host: host["id"], cluster["hosts"]):
            with suppressAndLog(assisted_service_client.rest.ApiException):
                client.download_host_ignition(
                    cluster["id"], host_id, os.path.join(output_folder, "cluster_files")
                )
        ...
        with suppressAndLog(assisted_service_client.rest.ApiException):
            ...
            shutil.copy2(
                os.path.join(
                    os.path.dirname(os.path.realpath(__file__)), "events.html"
                ),
                output_folder,
            )
        ...
        with suppressAndLog(assisted_service_client.rest.ApiException):
            ...
            is_controller_expected = (
                cluster["status"] == ClusterStatus.INSTALLED
                or are_masters_in_configuring_state
            )
        ...
        with suppressAndLog(assisted_service_client.rest.ApiException):
            ...
            if must_gather:
                ...
                download_must_gather(
                    kubeconfig_path, os.path.join(output_folder, "must-gather")
                )
    finally:
...
def write_metadata_file(client: InventoryClient, cluster: dict, file_name: str):
    ...
    with suppress(KeyError):
        d[
            "link"
        ] = f"{get_ui_url_from_api_url(client.inventory_url)}/clusters/{cluster['id']}"
...
def handle_arguments():
    ...
    parser.add_argument(
        "--cluster-id",
        help="Cluster id to download its logs",
        type=str,
        default=None,
        nargs="?",
    )
    parser.add_argument(
        "--download-all", help="Download logs from all clusters", action="store_true"
    )
    parser.add_argument("--must-gather", help="must-gather logs", action="store_true")
    parser.add_argument(
        "--update-by-events",
        help="Update logs if cluster events were updated",
        action="store_true",
    )
