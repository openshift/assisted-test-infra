...
private_ssh_key_path_default = (
    os.path.join(os.getcwd(), "ssh_key/key")
    if not qe_env
    else os.path.join(str(Path.home()), ".ssh/id_rsa")
)

env_variables = {
    "ssh_public_key": utils.get_env("SSH_PUB_KEY"),
    ...
    "storage_pool_path": utils.get_env(
        "STORAGE_POOL_PATH", os.path.join(os.getcwd(), "storage_pool")
    ),
    "cluster_name": _get_cluster_name(),
    "private_ssh_key_path": utils.get_env(
        "PRIVATE_KEY_PATH", private_ssh_key_path_default
    ),
...
if not qe_env:
    ...
    image = utils.get_env(
        "ISO",
        os.path.join(
            consts.IMAGE_FOLDER, f'{env_variables["cluster_name"]}-installer-image.iso'
        ),
    ).strip()
...
def get_api_client(offline_token=env_variables["offline_token"], **kwargs):
    ...
    if not url:
        url = utils.get_local_assisted_service_url(
            utils.get_env("PROFILE"),
            env_variables["namespace"],
            "assisted-service",
