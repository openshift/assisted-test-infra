...
class BaseTest:
    ...
    def cluster(self, api_client, request, nodes):
        ...
        def get_cluster_func(
            ...
            if not cluster_name:
                cluster_name = env_variables.get(
        ...
        def set_iptables_rules_for_nodes(
            ...
            if download_image:
                cluster.generate_and_download_image(
                    iso_download_path=iso_download_path, ssh_key=ssh_key
                )
    ...
    def attach_interface(self):
        ...
        def add(node, network_name=None, network_xml=None):
            ...
            added_networks.append(
                {"node": node, "network": network, "mac": interface_mac}
            )
    ...
    def assert_cluster_validation(
        cluster_info, validation_section, validation_id, expected_status
    ):
        found_status = infra_utils.get_cluster_validation_value(
            cluster_info, validation_section, validation_id
        )
    ...
    def collect_test_logs(self, cluster, api_client, test, nodes):
        ...
        with suppress(ApiException):
            cluster_details = json.loads(
                json.dumps(cluster.get_details().to_dict(), sort_keys=True, default=str)
            )
            download_logs(
                api_client, cluster_details, log_dir_name, test.result_call.failed
            )
    ...
    def _collect_virsh_logs(cls, nodes, log_dir_name):
        ...
        infra_utils.run_command(
            f"virsh net-list --all >> {libvirt_net_list_path}", shell=True
        )

        network_name = nodes.get_cluster_network()
        virsh_leases_path = os.path.join(virsh_log_path, "net_dhcp_leases")
        infra_utils.run_command(
            f"virsh net-dhcp-leases {network_name} >> {virsh_leases_path}", shell=True
        )
        ...
        for node in nodes:
            shutil.copy(
                f"/var/log/libvirt/qemu/{node.name}.log",
        ...
        for node in nodes:
            shutil.copy(
                f"/var/log/libvirt/qemu/{node.name}-console.log",
    ...
    def _collect_journalctl(nodes, log_dir_name):
        ...
        infra_utils.recreate_folder(
            log_dir_name, with_chmod=False, force_recreate=False
        )
