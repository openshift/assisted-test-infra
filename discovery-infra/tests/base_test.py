...
class BaseTest:
    ...
    @classmethod
    def nat_interfaces(cls, config: TerraformConfig) -> List[str]:
        return config.net_asset.libvirt_network_interfaces
    ...
    def _set_up_proxy_server(cluster: Cluster, cluster_config: ClusterConfig, proxy_server):
        ...
        no_proxy += cluster.get_machine_networks()
        no_proxy += [f".{str(cluster_config.cluster_name)}.redhat.com"]
        no_proxy = ",".join(no_proxy)

        host_ip = str(IPNetwork(cluster.get_primary_machine_cidr()).ip + 1)
