...
class TestKubeAPI(BaseKubeAPI):
    ...
    def kube_test_configs_single_node(self, request: FixtureRequest, cluster_configuration, controller_configuration):
        ...
        self._configure_ipv6(cluster_configuration, controller_configuration)

        yield cluster_configuration, controller_configuration

    @classmethod
    def _configure_ipv6(cls, cluster_configuration, controller_configuration):
        cluster_configuration.is_ipv6 = True
        controller_configuration.is_ipv6 = True

        cluster_configuration.is_ipv4 = False
        controller_configuration.is_ipv4 = False

        cluster_configuration.cluster_networks = consts.DEFAULT_CLUSTER_NETWORKS_IPV6
        cluster_configuration.service_networks = consts.DEFAULT_SERVICE_NETWORKS_IPV6
        cluster_configuration.vip_dhcp_allocation = False
        cluster_configuration.user_managed_networking = True
        cluster_configuration.high_availability_mode = consts.HighAvailabilityMode.NONE
        cluster_configuration.network_type = consts.NetworkType.OVNKubernetes
