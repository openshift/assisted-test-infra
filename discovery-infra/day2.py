...
def execute_day2_flow(cluster_id, args, day2_type_flag, has_ipv4):
    ...
    client.generate_and_download_image(
        cluster_id=cluster.id, image_path=image_path, ssh_key=args.ssh_key,
    )
...
def day2_nodes_flow(
    ...
    if install_cluster_flag:
        ...
        [
            client.install_day2_host(cluster.id, host["id"])
            for host in hosts
        ...
        log.info(
            "%d worker nodes were successfully added to OCP cluster", num_worker_nodes
        )
...
def set_hostnames_from_tf(client, cluster_id, tf_folder, network_name):
    ...
    utils.update_hosts(
        client, cluster_id, libvirt_nodes, update_roles=False, update_hostnames=True
    )


def apply_day2_tf_configuration(
...
def get_network_nodes_from_terraform(tf_folder):
    tfvars = utils.get_tfvars(tf_folder)
    return (
        tfvars["libvirt_network_name"],
        tfvars["master_count"] + tfvars["worker_count"],
    )


def set_workers_addresses_by_type(
    tfvars, num_worker_nodes, master_ip_type, worker_ip_type, worker_mac_type
):
    ...
    tfvars[worker_mac_type] = old_worker_mac_addresses + static_ips.generate_macs(
        num_worker_nodes
    )
...
def approve_workers_on_ocp_cluster():
    ...
    for csr in csrs:
        if not csr["status"]:
            ...
            log.info(
                "CSR %s for node %s has been approved",
                csr_name,
                csr["spec"]["username"],
            )
...
def get_ocp_cluster_nodes():
    res = subprocess.check_output(
        "oc --kubeconfig=build/kubeconfig get nodes --output=json", shell=True
    )
...
def get_ocp_cluster_csrs():
    res = subprocess.check_output(
        "oc --kubeconfig=build/kubeconfig get csr --output=json", shell=True
    )
    return json.loads(res)["items"]
...
def ocp_cluster_csr_approve(csr_name):
    subprocess.check_output(
        "oc --kubeconfig=build/kubeconfig adm certificate approve %s" % csr_name,
        shell=True,
    )
...
def set_cluster_proxy(client, cluster_id, copy_proxy_from_cluster, args):
    ...
    """
    http_proxy = (
        args.http_proxy if args.http_proxy else copy_proxy_from_cluster.http_proxy
    )
    https_proxy = (
        args.https_proxy if args.https_proxy else copy_proxy_from_cluster.https_proxy
    )
