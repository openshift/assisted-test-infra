...
def _find_free_cidr_machine(start_cidr_machine):
    ...
    routes, _, _ = utils.run_command('/usr/sbin/ip route show')
...
def _ping(ip):
    _, _, ret_code = utils.run_command(
        f'ping -c1 -w1 -i0.2 {ip}',
        raise_errors=False
    )
    return ret_code == 0
...
def _find_free_network_bridge():
    links, _, _ = utils.run_command(f'/usr/sbin/ip link show', shell=False)
    i = 0
