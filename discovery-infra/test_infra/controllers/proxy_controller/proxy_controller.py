...
class ProxyController:
    def __init__(
        self, name=None, port=None, denied_port=None, authenticated=False, dir=None
    ):
        if not name:
            self.address = ""
        else:
    ...
    def _set_server_address(self):
        ...
        proxy_user_path = (
            f"{PROXY_USER}:{PROXY_USER_PASS}@" if self.authenticated else ""
        )
    ...
    def _create_conf_from_template(self, denied_port):
        ...
        env = Environment(
            loader=PackageLoader("test_infra.controllers.proxy_controller", "templates")
        )
        template = env.get_template("squid.conf.j2")
        config = template.render(
            port=self.port, denied_port=denied_port, authenticated=self.authenticated
        )
