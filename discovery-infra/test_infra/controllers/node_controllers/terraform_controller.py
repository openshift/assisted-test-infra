...
class TerraformController(LibvirtController):
    def __init__(self, **kwargs):
        ...
        self.cluster_name = kwargs.get(
            "cluster_name", f"{consts.CLUSTER_PREFIX}" + "-" + self.cluster_suffix
        )
        self.network_name = (
            kwargs.get("network_name", consts.TEST_NETWORK) + self.cluster_suffix
        )
    ...
    def _terraform_params(self, **kwargs):
        params = {
            ...
            + self.cluster_suffix,
            "libvirt_storage_pool_path": kwargs.get(
                "storage_pool_path", os.path.join(os.getcwd(), "storage_pool")
            ),
            # TODO change to namespace index
    ...
    def _fill_tfvars(self, running=True):
        ...
        master_starting_ip = str(
            ipaddress.ip_address(ipaddress.ip_network(machine_cidr).network_address)
            + 10
        )
        ...
        tfvars["libvirt_master_macs"] = static_ips.generate_macs(
            self.params.master_count
        )
        tfvars["libvirt_worker_macs"] = static_ips.generate_macs(
    ...
    def _secondary_tfvars(self):
        ...
        return {
            ...
            "libvirt_secondary_master_macs": static_ips.generate_macs(
                self.params.master_count
            ),
            "libvirt_secondary_worker_macs": static_ips.generate_macs(
                self.params.worker_count
            ),
        }
    ...
    def format_node_disk(self, node_name):
        ...
        self.format_disk(
            f"{self.params.libvirt_storage_pool_path}/{self.cluster_name}/{node_name}"
        )
    ...
    def _create_address_list(self, num, starting_ip_addr):
        return (
            utils.create_empty_nested_list(num)
            if self.ipv6
            else utils.create_ip_address_nested_list(
    ...
    def get_machine_cidr(self):
        return (
            self.network_conf.machine_cidr6
            if self.ipv6
            else self.network_conf.machine_cidr
        )

    def _get_provisioning_cidr(self):
        return (
            self.network_conf.provisioning_cidr6
            if self.ipv6
            else self.network_conf.provisioning_cidr
        )
    ...
    def _delete_virsh_resources(cls, *filters):
        ...
        virsh_cleanup.clean_virsh_resources(
            skip_list=skip_list, resource_filter=filters
        )
    ...
    def prepare_nodes(self):
        ...
        if not os.path.exists(self.image_path):
            utils.recreate_folder(
                os.path.dirname(self.image_path), force_recreate=False
            )
