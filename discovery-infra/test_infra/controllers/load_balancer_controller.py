...
class LoadBalancerController:
    ...
    def _render_port_entities(self, load_balancer_ip, master_ips, port):
        ...
        return self._render_upstream_block(
            master_ips, port, upstream_name
        ) + self._render_server_block(load_balancer_ip, port, upstream_name)
    ...
    def set_load_balancing_config(self, load_balancer_ip, master_ips):
        load_balancer_config_file = self._render_load_balancer_config_file(
            load_balancer_ip, master_ips
        )
        self._tf.change_variables(
            {
                "load_balancer_ip": load_balancer_ip,
                "load_balancer_config_file": load_balancer_config_file,
            }
        )
