        ...
        with tarfile.open(cluster_tar_path) as tar:
            ...
            for gz in os.listdir(tempdir):
                if "bootstrap" in gz:
                    ...
                    _verify_bootstrap_logs_uploaded(
                        tempdir, gz, installation_success, verify_control_plane
                    )
...
def wait_and_verify_oc_logs_uploaded(
    cluster, cluster_tar_path, num_retries=NUM_OF_RETRIES
):
    for retry in range(num_retries):
        try:
            ...
            logging.info(
                f"attempt {retry + 1} to download failed with error {str(err)}"
            )
...
def _check_entry_from_extracted_tar(component, tarpath, verify):
    with TemporaryDirectory() as tempdir:
        ...
        with tarfile.open(tarpath) as tar:
            ...
            assert any(
                component in logfile for logfile in extractedfiles
            ), f"can not find {component} in logs"
            component_tar = [
                logfile for logfile in extractedfiles if component in logfile
            ][0]
...
def _verify_oc_logs_uploaded(cluster_tar_path):
    _check_entry_from_extracted_tar(
        "controller",
        cluster_tar_path,
        lambda path: _check_entry_from_extracted_tar(
            "must-gather", path, lambda inner: None
        ),
    )
...
def _verify_bootstrap_logs_uploaded(
    dir_path, file_path, installation_success, verify_control_plane=False
):
    gz = tarfile.open(os.path.join(dir_path, file_path))
    logs = gz.getnames()
    assert any(
        "bootkube.logs" in s for s in logs
    ), f"bootkube.logs isn't found in {logs}"
    if not installation_success:
        for logs_type in ["dmesg.logs", "log-bundle"]:
            assert any(
                logs_type in s for s in logs
            ), f"{logs_type} isn't found in {logs}"
        ...
        if verify_control_plane:
            ...
            logging.info(
                f"control-plane directory has sub-directory for each master: {master_dirs}"
            )
            for ip_dir in master_dirs:
                logging.info(
                    f"{ip_dir} content: {os.listdir(os.path.join(cp_full_path, ip_dir))}"
                )
                assert (
                    len(os.listdir(os.path.join(cp_full_path, ip_dir))) > 0
                ), f"{cp_path}/{ip_dir} is empty"
        lb.close()
    gz.close()
    ...
    for collected_at in logs_collected_at:
        ...
        if collected_at > time.time() - 86400000:
            assert collected_at > started_cluster_install_at, (
                f"logs collected at {collected_at}"
