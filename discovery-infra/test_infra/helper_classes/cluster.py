...
class Cluster:
    def __init__(
        ...
        if cluster_id:
            ...
            cluster_name = cluster_name or env_variables.get(
    ...
    def wait_until_hosts_are_discovered(
        self, nodes_count=env_variables["num_nodes"], allow_insufficient=False
    ):
    ...
    def _get_matching_hosts(self, host_type, count):
        hosts = self.get_hosts()
        return [
            {"id": h["id"], "role": host_type}
            for h in hosts
            if host_type in h["requested_hostname"]
        ][:count]
    ...
    def set_host_roles(self, requested_roles=None):
        if requested_roles is None:
            requested_roles = Counter(
                master=env_variables["num_masters"], worker=env_variables["num_workers"]
            )
        assigned_roles = self._get_matching_hosts(
            host_type=consts.NodeRoles.MASTER, count=requested_roles["master"]
        )

        assigned_roles.extend(
            self._get_matching_hosts(
                host_type=consts.NodeRoles.WORKER, count=requested_roles["worker"]
            )
        )

        self.api_client.update_hosts(
            cluster_id=self.id, hosts_with_roles=assigned_roles
        )
    ...
    def set_specific_host_role(self, host, role):
        ...
        self.api_client.update_hosts(
            cluster_id=self.id, hosts_with_roles=assignment_role
        )

    def set_network_params(
        self, controller, vip_dhcp_allocation=env_variables["vip_dhcp_allocation"],
    ):
        ...
        if (
            vip_dhcp_allocation
            or self._high_availability_mode == consts.HighAvailabilityMode.NONE
        ):
    ...
    def set_machine_cidr(self, machine_cidr):
        logging.info(
            f"Setting Machine Network CIDR:{machine_cidr} for cluster: {self.id}"
        )
        self.api_client.update_cluster(self.id, {"machine_network_cidr": machine_cidr})

    def set_ingress_and_api_vips(self, vips):
        logging.info(
            f"Setting API VIP:{vips['api_vip']} and ingres VIP:{vips['ingress_vip']} for cluster: {self.id}"
        )
    ...
    def set_additional_ntp_source(self, ntp_source: List[str]):
        ...
        if isinstance(ntp_source, List):
            ...
            raise TypeError(
                f"ntp_source must be a string or a list of strings, got: {ntp_source},"
                f" type: {type(ntp_source)}"
            )
        self.api_client.update_cluster(
            self.id, {"additional_ntp_source": ntp_source_string}
        )
    ...
    def wait_for_hosts_stage(
        self,
        stage: str,
        nodes_count: int = env_variables["num_nodes"],
        inclusive: bool = True,
    ):
        ...
        utils.wait_till_at_least_one_host_is_in_stage(
            client=self.api_client,
            cluster_id=self.id,
            stages=consts.all_host_stages[index:]
            if inclusive
            else consts.all_host_stages[index + 1 :],
            nodes_count=nodes_count,
        )
    ...
    def reboot_required_nodes_into_iso_after_reset(self, nodes):
        ...
        nodes.run_for_given_nodes_by_cluster_hosts(
            cluster_hosts=hosts_to_reboot, func_name="reset"
        )
    ...
    def prepare_for_install(
        ...
        self.wait_until_hosts_are_discovered(
            nodes_count=nodes_count, allow_insufficient=True
        )
        ...
        self.set_network_params(
            controller=nodes.controller, vip_dhcp_allocation=vip_dhcp_allocation,
        )
    ...
    def download_kubeconfig(self, kubeconfig_path=env_variables["kubeconfig_path"]):
    ...
    def host_update_install_progress(self, host_id, current_stage, progress_info=None):
        self.api_client.host_update_progress(
            self.id, host_id, current_stage, progress_info=progress_info
        )

    def host_complete_install(self):
        self.api_client.complete_cluster_installation(
            cluster_id=self.id, is_success=True
        )
    ...
    def is_cluster_validation_in_status(
        ...
        logging.info(
            "Is cluster %s validation %s in status %s", self.id, validation_id, statuses
        )
        try:
            return (
                utils.get_cluster_validation_value(
    ...
    def is_host_validation_in_status(
        ...
        logging.info(
            "Is host %s validation %s in status %s", host_id, validation_id, statuses
        )
        try:
            return (
                utils.get_host_validation_value(
            ...
            if event_to_find in event["message"]:
                ...
                if utils.to_utc(event["event_time"]) >= reference_time - 2:
                    if all(param in event["message"] for param in params_list):
                        # event_exist = True
                        logging.info(
                            f"Event to find: {event_to_find} exists with its params"
                        )
                        return True
        else:
            return False
    ...
    def wait_for_event(
        self, event_to_find, reference_time, params_list=None, host_id="", timeout=10
    ):
        ...
        try:
            waiting.wait(
                lambda: self._find_event(
                    event_to_find, reference_time, params_list, host_id
                ),
                timeout_seconds=timeout,
                sleep_seconds=2,
    ...
    def get_hosts_nics_data(hosts: list, ipv4_first=True):
        return [
            Cluster.get_inventory_host_nics_data(h, ipv4_first=ipv4_first)
            for h in hosts
        ]
    ...
    def get_ip_for_single_node(client, cluster_id, machine_cidr, ipv4_first=True):
        ...
        interfaces = Cluster.get_inventory_host_nics_data(
            cluster_info["hosts"][0], ipv4_first=ipv4_first
        )
    ...
    def get_master_ips(client, cluster_id, network):
        ...
        hosts_interfaces = Cluster.get_hosts_nics_data(
            [h for h in cluster_info["hosts"] if h["role"] == consts.NodeRoles.MASTER]
        )
    ...
    def get_api_vip(self, cluster):
        ...
        if not api_vip and cluster.user_managed_networking:
            ...
            masters = self.get_hosts_by_role(
                consts.NodeRoles.MASTER, hosts=cluster.to_dict()["hosts"]
            )
    ...
    def _wait_for_api_vip(self, hosts, timeout=180):
        ...
        return waiting.wait(
            lambda: self.get_kube_api_ip(hosts=hosts),
            timeout_seconds=timeout,
            sleep_seconds=5,
    ...
    def is_kubeapi_service_ready(ip_or_dns):
        ...
        try:
            response = requests.get(
                f"https://{ip_or_dns}:6443/readyz", verify=False, timeout=1
            )
            return response.ok
        except BaseException:
            return False
