...
class Cluster:
    ...
    def get_machine_networks(self):
        networks = self.nodes.controller.get_machine_networks()
    ...
    def get_primary_machine_cidr(self):
        return self.get_machine_networks()[0]
    ...
    def _configure_load_balancer(self):
        ...
        master_ips = []
        worker_ips = []
        for network_cidr in machine_networks:
            master_ips += self.get_master_ips(self.api_client, self.id, network_cidr)
            worker_ips = self.get_worker_ips(self.api_client, self.id, network_cidr)

        load_balancer_ip = str(IPNetwork(machine_networks[0]).ip + 1)
