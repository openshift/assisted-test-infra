...
def _are_operators_in_status(operators: List[MonitoredOperator], operators_count: int, statuses: List[str],
                             fall_on_error_status: bool) -> bool:
    log.info(
    ...
    if fall_on_error_status:
        for operator in operators:
            if operator.status == consts.OperatorStatus.FAILED:
                raise ValueError(
                    f"Operator {operator.name} status is {consts.OperatorStatus.FAILED} with info {operator.status_info}")
...
def filter_operators_by_type(operators: List[MonitoredOperator], operator_types: List[str]) -> \
        List[MonitoredOperator]:
